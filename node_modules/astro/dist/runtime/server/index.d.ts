export { createComponent } from 'astro/dist/runtime/server/astro-component.js';
export { createAstro } from 'astro/dist/runtime/server/astro-global.js';
export { renderEndpoint } from 'astro/dist/runtime/server/endpoint.js';
export { escapeHTML, HTMLBytes, HTMLString, markHTMLString, unescapeHTML } from 'astro/dist/runtime/server/escape.js';
export { renderJSX } from 'astro/dist/runtime/server/jsx.js';
export { addAttribute, addScopeFlag, createHeadAndContent, createScopedResult, defineScriptVars, Fragment, maybeRenderHead, removeScopeFlag, renderAstroTemplateResult as renderAstroComponent, renderComponent, renderComponentToIterable, Renderer as Renderer, renderHead, renderHTMLElement, renderPage, renderScriptElement, renderSlot, renderStyleElement, renderTemplate as render, renderTemplate, renderToString, renderUniqueStylesheet, ScopeFlags, stringifyChunk, voidElementNames, } from 'astro/dist/runtime/server/render/index.js';
export type { AstroComponentFactory, AstroComponentInstance, ComponentSlots, RenderInstruction, } from 'astro/dist/runtime/server/render/index.js';
export declare function mergeSlots(...slotted: unknown[]): Record<string, () => any>;
/** @internal Associate JSX components with a specific renderer (see /src/vite-plugin-jsx/tag.ts) */
export declare function __astro_tag_component__(Component: unknown, rendererName: string): void;
export declare function spreadAttributes(values: Record<any, any>, _name?: string, { class: scopedClassName }?: {
    class?: string;
}): any;
export declare function defineStyleVars(defs: Record<any, any> | Record<any, any>[]): any;
